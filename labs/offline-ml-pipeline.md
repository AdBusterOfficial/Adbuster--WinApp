...
The entire workflow — recording, feature extraction, CSV generation, model training and deployment — runs fully offline inside AdusterML, producing a stable `ad_detector.pkl` used for further research and prototyping.

Initial evaluation shows an 87% accuracy and a weighted F1‑score of 0.87, with perfect recall for AD detection — confirming that the model generalizes well even on a small, controlled dataset. These results validate the stability of the MFCC‑based feature space and the consistency of the RandomForest decision paths, making the current `ad_detector.pkl` a reliable baseline for future dataset expansion and multi‑class audio logic.
...
This pipeline is **not part of the main AdBuster PRO application**.  
It is a **research‑only environment** used to validate offline DSP/ML workflows, test MFCC feature stability, and prototype future multi‑class audio logic.
